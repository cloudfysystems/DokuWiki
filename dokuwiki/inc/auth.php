...
function auth_login($user, $pass, $sticky = false, $silent = false) {
    ...
    if(!empty($user)) {
        ...
        if(!empty($pass) && $auth->checkPass($user, $pass)) {
			...
			//echo '<pre>';var_dump($_SESSION[DOKU_COOKIE]);'</pre>';die();
            return true;
        } else {
...
function auth_setCookie($user, $pass, $sticky) {
    ...
    if(!$auth) return false;
	//se esta logando com token, usa o token para setar as informacoes de sessao e o cookie
	if ($user == '@token@') {
		//O $pass chega codificado aqui, entao temos que decodificalo para ter o token
		$secret = auth_cookiesalt(!$sticky, true); //bind non-sticky to session
        $pass_tmp   = auth_decrypt($pass, $secret);
		list($USERINFO, $_tmp) = $auth->getTokenData($pass_tmp);//seta USERINFO e coloca a senha igual ao cod de verificacao do jwt
		$pass = auth_encrypt($_tmp, $secret);//reencripta a senha para enviar para o cookie
		$user = $USERINFO['user'];//coloca o usuario real do login(pego do token)
