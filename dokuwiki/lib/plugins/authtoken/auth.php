<?php
/**
 * DokuWiki Plugin authtoken (Auth Component)
 *
...
/**
 * Class auth_plugin_authtoken
 */
class auth_plugin_authtoken extends DokuWiki_Auth_Plugin {
    ...
    public function checkPass($user, $pass) {
		//verifica se esta fazendo login usando token(POST enviado pela retaguarda)
		if($user != "@token@") return false;
		if($pass == null) return false;
		
		//decodificando o token
		$cloudfy_param =  str_replace('/lib/plugins/authtoken', '/conf/cloudfy.json', str_replace('\\', '/', __DIR__));
		$key = file_get_contents($cloudfy_param);
		$key = json_decode($key);
		$key = $key->path_jwt;
		$key = file_get_contents($key);
		
		$decoded = JWT::decode($pass, $key, array('HS512'));//retorna o conteudo do token se a verificacao do token
		if($decoded === false) return false;//se deu erro na decodificacao do Token
		
		//transforma o payload decodificado em json
		$decoded = json_decode($decoded->sub);
		
		//setando userdata
		$userdata["uid"] = (string) $decoded->UserNr;
		$userdata["user"] = $decoded->UserLogin;
		$userdata["name"] = $decoded->UserName;
		$userdata["hash"] = $decoded->UserLogin;
		$userdata["mail"] = $decoded->UserEmail;
		$userdata["branch"] = $decoded->CompanyBranch->BranchDescription;
	
		//se qualquer um dos dados do usuario nao for setado, falha o login
		foreach($userdata as $ud_val) {
			if($ud_val == null)return false;
		}	
        return true;
    }
    ...
    protected function _debug($message, $err = 0, $line = 0) {
        ...
        if(defined('DOKU_UNITTEST')) {
            ...
            msg('authtoken: ' . $msg, $err, $line, __FILE__);
        }
    }
	...
	/**
     * decodifica o token para pegar os parametros do usuario
     *
     * @param string $pass (contem o token)
     */
	public function getTokenData($pass) {		
		if($pass == null) return false;
		...
		$decoded = JWT::decode($pass, $key, array('HS512'));//retorna o conteudo do token se a verificacao do token
		if($decoded === false) return false;//se deu erro na decodificacao do Token
		$pass = base64_decode(explode(".",$pass)[2]);//passa a chave de verificacao como a nova senha para guardar no cookie de sessao
		//transforma o payload decodificado em json
		$decoded = json_decode($decoded->sub);
		...
		$userdata["mail"] = $decoded->UserEmail;
		$userdata["branch"] = $decoded->CompanyBranch->BranchDescription;
		$userdata["grps"] = array('cliente');
			
        return array($userdata,$pass);//passa a chave de verificacao como a nova senha para guardar no cookie de sessao
    }
}
